...
def run_test_draw_points_on_circle():
    ...
    # -------------------------------------------------------------------------
    # Test 1:
    title = 'DRAW_POINTS_ON_CIRCLE, test 1:  1000 light green dots.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    draw_points_on_circle(window, circle, 1000, 'light green')
    window.close_on_mouse_click()
...
def run_test_pizza():
    ...
    # -------------------------------------------------------------------------
    title = 'PIZZA test 1:  400 slices, thin (thickness=1) light green lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.outline_thickness = 3
    circle.fill_color="black"
    pizza(window, circle, 400, 'light green', 1)
    window.close_on_mouse_click()
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for i in range(len(points)):
        line=rg.Line(circle.center,points[i])
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    pi=points[0]
    pe=points[number_of_segments-1]
    line2=rg.Line(pi,pe)
...
def run_test_fancy_polygon():
    ...
    circle = rg.Circle(rg.Point(240, 165), 150)
    circle.fill_color = 'blue'
    fancy_polygon(window, circle, 51, 25, 'lime green', 1)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color,
    ...
    for k in range(hops_to_next_point):
        pi=points[k]
        pe=points[number_of_lines-hops_to_next_point+k]
        line2=rg.Line(pe,pi)
        line2.attach_to(window)
        line2.color=color
        line2.thickness=thickness
    ...
    for i in range(len(points)-hops_to_next_point):

        p1=points[i]
        p2=points[i+hops_to_next_point]
        line=rg.Line(p1,p2)
        line.thickness=thickness
        line.color=color
        line.attach_to(window)
